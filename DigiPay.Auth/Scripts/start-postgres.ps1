...
while ($attempts -lt $maxAttempts) {
    try {
        docker exec digipay-postgres pg_isready > $null
...
Write-Host "Conexão: Host=localhost;Port=5432;Database=digipay_auth;Username=postgres;Password=postgres" -ForegroundColor Cyan 